...
from nltk.translate.bleu_score import sentence_bleu  # 需要安装nltk
import numpy as np
...
class TrainModel(object):
    ...
    def train(self, data_loader, model, optimizer, criterion, char_to_ix, ix_to_chars):
        for epoch in range(self.config.epoch_num):
            for step, x in enumerate(data_loader):
                ...
                optimizer.zero_grad()
                # input,target:  (batch_size, max_len - 1)
                input_, target = x[:, :-1], x[:, 1:]
                # 确保target是二维的
                target = target.reshape(-1, target.size(-1))  # 重塑target为(batch_size, seq_len)
                # 初始化hidden为(c0, h0): ((layer_num， batch_size, hidden_dim)，(layer_num， batch_size, hidden_dim)）
                batch_size = x.size()[0]
                hidden = model.init_hidden(batch_size, self.config.layer_num)

                # 2.前向计算
                output, hidden = model(input_, target, hidden)  # 确保传递target参数
                loss = criterion(output.view(-1, output.size(-1)), target.view(-1))  # output:(batch_size*max_len, vocab_size), target:(batch_size*max_len)
    ...
    def generate_head_test(self, model, head_sentence, word_to_ix, ix_to_word):
        ...
        hidden = model.init_hidden(1, self.config.layer_num)

        for i in range(self.config.max_gen_len):
            # 前向计算出概率最大的当前词
            output, hidden = model(input, None, hidden)  # 注意这里的修改
            ...
            if pre_char in ['。', '！', '<START>']:
                if sentence_len == head_char_len:
                    ...
                    sentence_len += 1
                    input = (torch.Tensor([word_to_ix[char]]).view(1, 1).long()).to(self.device)
            else:
    ...
    def evaluate(self, model, char_to_ix, ix_to_chars):
        # 评估逻辑，可以根据需要实现
        pass
...
if __name__ == '__main__':
    obj = TrainModel()
    obj.run()
    if obj.config.evaluation_mode:
        obj.evaluate(obj.model, obj.char_to_ix, obj.ix_to_chars)